...
function validateOrderUser(data) {
  ...
  if (!['ATM', 'Credit Card', 'Apple Pay'].includes(data.payment)) errors.push('付款方式不合法');
...
1. axios 會自動解析 JSON，而 fetch 需要手動呼叫 .json()。
2. axios 支援請求和回應的攔截器（interceptors），fetch 需要額外封裝。
3. axios 預設會處理跨瀏覽器的錯誤，而 fetch 需要手動處理 HTTP 錯誤。
*/
